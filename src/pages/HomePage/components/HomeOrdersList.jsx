...
const HomeOrdersList = (props) => {    
    ...
    return (
        <div className='flex flex-col justify-end items-center w-full max-w-xl sm:max-w-2xl md:max-w-3xl'>
            ...
            <div className='grid h-full w-full max-h-100 md:max-h-120 px-4 md:px-8 overflow-auto'>
                {orders.map((item, index) => 
                    <div onClick={() => setPickedOrder(item.id)} tabIndex={index} className='focus:border-violet-700 focus:border-2 flex flex-col items-start max-h-16 w-full p-1 border border-gray-400 rounded-sm mb-3'>
